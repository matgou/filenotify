...
filenotify_mainloop ()
{
  ...
  if (engine_fd == -1)
    {
      log_msg ("ERROR", "Error while init engine %s : ", FILENOTIFY_ENGINE,
	       strerror (errno));
      exit (EXIT_FAILURE);
    }
 ...
 */
void
filenotify_execplugins (directory_t * dir, plugin_arg_t * event_)
{
 ...
 */
void *
filenotify_execplugin (void *ptrc)
{
  // decode ptr
  ...
  for (np = plugins_config; np != NULL; np = np->next)
    {
      ...
      int plugin_path_len =
	strlen (plugin_name) + strlen (config_getbykey ("plugins_dir")) + 1;
 ...
 */
void
filenotify_sighandler (int signo)
{
  if (signo == SIGUSR1)
    {
 ...
 */
void
filenotify_directory_free (directory_t * l)
{
  if (l == NULL)
    {
      return;
 ...
 */
void
filenotify_plugins_free (plugin_t * l)
{
  if (l != NULL)
    {
      if (l->next != NULL)
